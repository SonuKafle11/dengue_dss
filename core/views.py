...
def patient_dashboard(request):
    ...
    return render(request, 'core/patient_dashboard.html', {'user': user, 'records': records})
...
def doctor_dashboard(request):
    ...
    if search: records = records.filter(patient__name__icontains=search)
    if status == 'reviewed': records = records.filter(is_reviewed=True)
    elif status == 'pending': records = records.filter(is_reviewed=False)
    return render(request, 'core/doctor_dashboard.html', {
        'user': user, 'records': records, 'search': search, 'status_filter': status,
...
def doctor_patient_detail(request, record_id):
    ...
    if request.method == 'POST':
        try:
            ...
            dosage_rec = recommend_dosage(
                weight_kg=rec.weight, age=rec.age,
    ...
    return render(request, 'core/doctor_patient_detail.html', {
        'record': rec, 'doctor': doctor, 'model_trained': is_model_trained()
    })
